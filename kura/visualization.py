...
# Standalone visualization functions for procedural API
def visualise_clusters_enhanced(
    ...
    Args:
        clusters: List of clusters to visualize (optional if checkpoint_path is provided)
        checkpoint_path: Path to checkpoint file containing clusters (optional if clusters is provided)
        console: Rich console instance (optional)
    """
    if clusters is None and checkpoint_path is None:
        raise ValueError("Either clusters or checkpoint_path must be provided")

    if clusters is None:
        # Load clusters from checkpoint
    ...
    node_id_to_cluster = {}
    total_conversations = sum(
        len(cluster.chat_ids) for cluster in clusters if not cluster.parent_id
    )
...
def visualise_clusters_rich(clusters: List[Cluster], console: Optional[Console] = None):
    """Print a rich-formatted hierarchical visualization using Rich library.

    Args:
        clusters: List of clusters to visualize
        console: Rich console instance (optional, will create one if not provided)
    """
    if not RICH_AVAILABLE:
        print("⚠️  Rich library not available. Using enhanced visualization...")
        visualise_clusters_enhanced(clusters=clusters)
        return
...
def visualise_from_checkpoint_manager(
    checkpoint_manager, meta_cluster_model, style: str = "basic"
):
    """Visualize clusters from checkpoint manager.

    Args:
        checkpoint_manager: CheckpointManager instance
        meta_cluster_model: Meta cluster model instance
    ...
    if style == "enhanced":
        ...
        # Load clusters for rich visualization
        with open(checkpoint_path) as f:
            clusters = [Cluster.model_validate_json(line) for line in f]
        visualise_clusters_rich(clusters)
    else:  # basic
        # Load clusters and use basic tree visualization
        with open(checkpoint_path) as f:
            clusters = [Cluster.model_validate_json(line) for line in f]
        _visualise_clusters_basic(clusters)


def visualise_pipeline_results(clusters: List[Cluster], style: str = "enhanced"):
    """Visualize clusters from pipeline results.

    Args:
        clusters: List of clusters from pipeline
        style: Visualization style ('basic', 'enhanced', or 'rich')
    """
    if style == "rich":
        visualise_clusters_rich(clusters)
...
def _visualise_clusters_basic(clusters: List[Cluster]):
    """Basic tree visualization of clusters."""
    node_id_to_cluster = {}
