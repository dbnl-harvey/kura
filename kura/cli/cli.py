...
def migrate_checkpoints(
    ...
    hub_repo: Optional[str] = typer.Option(
        None,
        help="Optional HuggingFace Hub repository name for uploading checkpoints"
    ),
    hub_token: Optional[str] = typer.Option(
        None,
        help="Optional HuggingFace Hub token for authentication"
    ),
    compression: str = typer.Option(
        "gzip",
        help="Compression algorithm to use (gzip, lz4, zstd, or none)"
    ),
    delete_source: bool = typer.Option(
        False,
        help="Delete source JSONL files after successful migration"
    ),
    verify: bool = typer.Option(
        True,
        help="Verify migration by comparing data"
    ),
):
    ...
    try:
        ...
        if successful == total:
            print(f"[bold green]✅ Migration complete: {successful}/{total} checkpoints migrated successfully[/bold green]")
        else:
            print(f"[bold yellow]⚠️  Migration partial: {successful}/{total} checkpoints migrated successfully[/bold yellow]")
        ...
        if verify and successful > 0:
            ...
            if verified == total_checkpoints:
                print(f"[bold green]✅ Verification complete: {verified}/{total_checkpoints} checkpoints verified[/bold green]")
            else:
                print(f"[bold yellow]⚠️  Verification partial: {verified}/{total_checkpoints} checkpoints verified[/bold yellow]")
...
def analyze_checkpoints(
    ...
    try:
        ...
        if stats["total_size_mb"] > 1:
            ...
            print(f"  Space savings: ~{stats['estimated_space_savings_mb']} MB ({int(stats['estimated_space_savings_mb']/stats['total_size_mb']*100)}%)")
