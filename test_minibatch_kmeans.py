...
def test_clustering_methods():
    ...
    print(f"Cluster sizes: min={min(standard_sizes)}, max={max(standard_sizes)}, avg={np.mean(standard_sizes):.1f}")
    
    # Test MiniBatch K-means
    print("\n--- Testing MiniBatch K-means ---")
    minibatch_kmeans = MiniBatchKmeansClusteringMethod(
        clusters_per_group=50,
        batch_size=100,
        max_iter=50,
        random_state=42
    )
    ...
    print(f"Cluster sizes: min={min(minibatch_sizes)}, max={max(minibatch_sizes)}, avg={np.mean(minibatch_sizes):.1f}")
    
    # Compare results
    print(f"\n--- Comparison ---")
    print(f"Standard K-means clusters: {len(standard_results)}")
    print(f"MiniBatch K-means clusters: {len(minibatch_results)}")
    print(f"Difference in cluster count: {abs(len(standard_results) - len(minibatch_results))}")
    ...
    large_minibatch_kmeans = MiniBatchKmeansClusteringMethod(
        clusters_per_group=100,
        batch_size=500,
        random_state=42
    )
    large_results = large_minibatch_kmeans.cluster(large_test_data)
    
    print(f"MiniBatch K-means processed {len(large_test_data)} items into {len(large_results)} clusters")
    large_sizes = [len(items) for items in large_results.values()]
    print(f"Large dataset cluster sizes: min={min(large_sizes)}, max={max(large_sizes)}, avg={np.mean(large_sizes):.1f}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"Use it for large datasets (100k+ conversations) to avoid memory issues.")
