...
async def run_pipeline_with_manager(
    ...
    console.print(f"✓ Dimensionality reduction: {len(projected_clusters)} projected clusters")
...
def create_comparison_table(jsonl_results: Dict, parquet_results: Dict) -> Table:
    ...
    for file_type in ['conversations', 'summaries', 'clusters', 'meta_clusters', 'dimensionality']:
        ...
        if jsonl_size > 0 and parquet_size > 0:
            improvement = f"{(jsonl_size - parquet_size) / jsonl_size * 100:.1f}% smaller"
            size_ratio = f"{jsonl_size / parquet_size:.1f}x"
        elif jsonl_size > 0:
            improvement = "N/A"
            size_ratio = "N/A"
        else:
    ...
    for step in ['summarization', 'clustering', 'meta_clustering', 'dimensionality']:
        ...
        if jsonl_time > 0 and parquet_time > 0:
            if parquet_time < jsonl_time:
                improvement = f"{(jsonl_time - parquet_time) / jsonl_time * 100:.1f}% faster"
            else:
                improvement = f"{(parquet_time - jsonl_time) / jsonl_time * 100:.1f}% slower"
        else:
            improvement = "N/A"
...
async def main():
    ...
    if not PARQUET_AVAILABLE:
        console.print("[red]❌ PyArrow not available. Please install with: pip install pyarrow[/red]")
        return
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Summary
        total_size_reduction = (jsonl_results['total_size'] - parquet_results['total_size']) / jsonl_results['total_size'] * 100
        
        console.print(f"\n[bold green]Summary:[/bold green]")
        console.print(f"• Parquet format is {total_size_reduction:.1f}% smaller than JSONL")
        console.print(f"• Total space saved: {format_size(jsonl_results['total_size'] - parquet_results['total_size'])}")
        
        if parquet_results['total_size'] > 0:
            compression_ratio = jsonl_results['total_size'] / parquet_results['total_size']
